...
It scans a filesystem root, analyzes metadata and bounded text samples, and produces Markdown/JSON reports. By default, the Markdown report is written to `~/codespace21/filesystem_audit.md`. It can also call any OpenAI-compatible chat-completions endpoint, including local Ollama-compatible endpoints, to generate an AI narrative over the local report.

## Quick start

```bash
python fs_ai_auditor.py /path/to/scan
```

Override the Markdown path when needed:
